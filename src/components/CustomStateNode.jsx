...
export const CustomStateNode = ({ data }) => {
  ...
  const { diffDict, rowData, activeState } = useAppContext();
    ...
    <div className="relative">
      <div
        className={`px-4 py-2 shadow-md rounded-md border-2 min-w-[120px] text-center ${
          data.isTarget ? "bg-blue-100 border-blue-500 text-blue-900" : "bg-white border-gray-300 text-gray-700"
        }`}
      >
        <div className="flex items-center justify-between">
          <span className="font-medium">{data.label}</span>
          <button onClick={() => setShowDropdown(!showDropdown)} className="ml-2 text-gray-500 hover:text-gray-700 focus:outline-none">
            ⋮
          </button>
        </div>
        ...
        {showDropdown && (
          <div className="absolute top-full right-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-10 min-w-[120px]">
            ...
            <button onClick={handleRevertDiff} className="block w-full px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100">
              Revert Diff
            </button>
          </div>
        )}
