...
const DiffPopup = ({ show, diffResults, selectedDiffs, onToggleDiff, onCopy, onClose, rowData }) => {
  ...
  const handleMetaChange = (entry, field, value) => {
    setMetadata((prev) => {
      ...
      updateMetadataFor(entry.containerId, entry.targetId, transitionLabel, direction, updated[entry.key]);
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full flex flex-col" style={{ maxHeight: "calc(100vh - 96px)" }}>
        ...
        <div className="flex-1 overflow-y-auto p-4 min-h-0">
          {diffEntries.length === 0 ? (
            <div className="text-gray-500 text-center py-8">No differences found</div>
          ) : (
            <div className="space-y-2">
              {diffEntries.map((entry) => (
                <label key={entry.key} className="flex items-start space-x-3 p-3 border border-gray-200 rounded hover:bg-gray-50 cursor-pointer">
                  <input
                    type="checkbox"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-gray-900 break-words">{entry.description}</div>
                    ...
                    <div className="mt-2 grid grid-cols-3 gap-2 text-xs">
                      <input
                        ...
                        onChange={(e) => setMetadata(prev => ({
                          ...prev,
                          [entry.key]: { ...prev[entry.key], weight: e.target.value }
                        }))}
                        onBlur={(e) => handleMetaChange(entry, "weight", e.target.value)}
                      ...
                      <input
                        ...
                        onChange={(e) => setMetadata(prev => ({
                          ...prev,
                          [entry.key]: { ...prev[entry.key], qual_label: e.target.value }
                        }))}
                        onBlur={(e) => handleMetaChange(entry, "qual_label", e.target.value)}
                        className="border border-gray-300 rounded px-1 py-0.5"
                      />
                      <input
                        type="text"
                        ...
                        onChange={(e) => setMetadata(prev => ({
                          ...prev,
                          [entry.key]: { ...prev[entry.key], notes: e.target.value }
                        }))}
                        onBlur={(e) => handleMetaChange(entry, "notes", e.target.value)}
                        className="border border-gray-300 rounded px-1 py-0.5"
                      />
                    </div>
                  </div>
          ...
          <div className="space-x-2">
            ...
            <button
              ...
              className={`px-4 py-2 text-sm rounded text-white ${
                selectedCount === 0 ? "bg-gray-300 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
              }`}
            >
